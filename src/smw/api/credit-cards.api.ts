...
export const apiNewCreditCard = async (newCreditCard: CreditCard): Promise<CreditCard> => {
	const { data } = await smwApiClient.post<ApiResponse<CreditCard>>(
		ApiEndpoints.CREDIT_CARDS,
		cleanCreditCard(newCreditCard)
	);
...
export const apiUpdateCreditCard = async (creditCard: CreditCard): Promise<CreditCard> => {
	const { data } = await smwApiClient.patch<ApiResponse<CreditCard>>(
		`${ApiEndpoints.CREDIT_CARDS}/${creditCard.id}`,
		cleanCreditCard(creditCard)
	);
